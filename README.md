...
Jarvis Reader 是一个面向 Obsidian 的个人化 EPUB 阅读插件。它把书架、目录导航、阅读进度、划线摘抄、读书笔记和双链感想整合到同一个阅读流程里。
...
本仓库主要记录我围绕个人读书流做的改造，包括书架与目录布局、摘抄侧栏、划线感想、Obsidian 双链输入、阅读进度显示和界面细节调整。
...
- 显示章节内页码和全书阅读百分比。
- 选中正文后创建划线、摘抄和感想。
- 将摘抄写入同名 Markdown 读书笔记。
- 在感想中使用 Obsidian 风格的 `[[双链]]`。
- 在摘抄预览中点击双链打开对应笔记。
- 在侧边栏搜索、筛选、排序摘抄。
...
阅读进度、划线摘抄、封面缓存、插件设置等本地数据可能由 Obsidian 存储在 `data.json` 中。该文件已被 `.gitignore` 排除，不应提交到 GitHub。
